...
mod tests {
    ...
    async fn run_command_success_and_failure() {
        ...
        assert!(matches!(execute_step(&step, &runner, &vault, &opener).await, StepOutcome::Failure { .. }));
    ...
    async fn spawn_error_becomes_failure_not_panic() {
        ...
        let StepOutcome::Failure { message, .. } = execute_step(&step, &runner, &vault, &opener).await else {
            panic!("Failure여야 함");
        };
    ...
    async fn path_check_uses_filesystem() {
        ...
        let vault = SecretVault::new();
        let opener = FakeUrlOpener::default();
        assert!(matches!(execute_step(&good, &runner, &vault, &opener).await, StepOutcome::Success { .. }));
        assert!(matches!(execute_step(&bad, &runner, &vault, &opener).await, StepOutcome::Failure { .. }));
    }
    ...
    async fn unwired_steps_report_unsupported() {
        ...
        assert!(matches!(execute_step(&step, &runner, &vault, &opener).await, StepOutcome::Unsupported));
    }
