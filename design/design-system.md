...
2. **긴고아 골드가 주 색** (사용자 확정, 2026-07-05). 주 버튼·선택 상태·진행바·강조 배너 전부 골드 그라디언트(primaryGradient)+goldButton 그림자. 청록(cloudMint/cloudDeep)은 근두운 모티프(정보성 포인트)에만 제한적으로.
3. **근두운 = 진행·이동** — 어려운 일은 마스코트가 "대신 날아가서" 하는 연출.
4. **UI 카피에 em dash(—) 금지.** 카피는 humanize 검수를 거친다 (AI 말투 제거). 이모지는 브랜드 요소가 아니므로 UI에 사용하지 않는다 (벨·체크 등은 도형/글리프로).
...
### 카탈로그 계층 규칙
- 카탈로그(하네스 탭)에는 **같은 계층인 하네스 6종만** 나열한다. 플러그인·오픈소스 5종은 사이드바 "플러그인 · 오픈소스" 메뉴로 분리.
- 도구 로고는 각 프로젝트 공식 이미지를 라이브 검증 후 사용 (확인: Anthropic·OpenAI·OpenClaw·NousResearch(Hermes)·Anomaly(OpenCode)). 공식 로고가 없는 도구(GajaeCode)는 이니셜 타일 + "준비 중" 표기.

### 버튼 위계
- 주(Primary): primaryGradient 배경 + onBrand 텍스트 + goldButton 그림자, radius.button. 화면당 1개 원칙.
