...
## 🔧 미커밋 수정 (2026-07-23) — 실사용 테스트로 찾은 출시 차단급 버그 3건

**제작자 본인이 v0.1.3 실제 dmg로 처음 실사용해보다 발견.** 코드 수정 완료, 커밋 전.
...
## 🔴 지금 상태 (2026-07-23) — 개발 동결. 노출 실험 대기 중
...
| | 이 글 | 채널 기준 |
|---|---|---|
| 조회 | **1,400** | 평균 약 4,000~6,000 (30일 108.4만) · 잘 되는 글 1.9만~19만 |
...
- [ ] **강사 발송** (미실시. 안 하면 B축 ⓐ 영구 미측정)
      - 링크에 `?utm_source=instructor` → 공개 홍보 경로와 분리
      - 아는 강사 2명: 기록만, **판정 제외**
      - **모르는 강사·교육기관 5곳 이상**: ⓐ는 여기서만 카운트
- [ ] **7/24 이후** `gh api repos/needsbuilder/easy-harness/traffic/views` + `/traffic/popular/referrers` 재조회 → 7/22 유입 복원
...
- [x] ~~채널 실측~~ **완료 (2026-07-23)**: 팔로워 4,069 · 30일 조회 108.4만 · 30일 반응 6,197
      → **게시물 평균 조회 약 4,000~6,000** ← 유효성 게이트 기준선
      → **평균 3배 = 조회 1.2만~1.8만** ← A축 ⓔ 기준선
      → 상위 5개 참고: 19만 / 5.3만 / 2.1만 / 2.0만 / 1.9만
- [x] ~~기준선 스냅샷~~ **완료 (2026-07-22 게시 직전 기준)**: 스타 0(외부) · dmg 1회(자체 검증) · 이슈 0
      → 1라운드 후 실측: 스타 외부 5 · 실다운로드 4 · 이슈 0
- [ ] **판정 기준 확정** (실측이 나왔으니 조정 가능. "기준 조정 4규칙" 준수)
      → 유도식: `ⓑ 다운로드 = 기대도달 R × 0.5%`. 평균 도달 5,000 기준이면 ⓑ ≈ 25회. **원안 20회 유지가 타당**
- [ ] **여기까지 HANDOFF에 적고 `git commit` + `git push`** ← 사전 등록 잠금. 푸시 안 하면 잠긴 게 아니다
- [ ] **모르는 강사·교육기관 5곳 명단** 작성 (인프런·패스트캠퍼스 AI 코딩 강사, AI 교육 스타트업, 대학·기관 특강 담당 등)
...
**0단계 유효성 게이트**: 도달이 자기 채널 최근 10개 평균 미만이면 **이번 라운드 무효**. 판정 안 하고 다른 채널로 다시. (없으면 "홍보가 약해서 실패"와 "수요가 없어서 실패"가 구분 안 됨)
