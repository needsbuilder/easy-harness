...
## 다음 스텝
1. **운영 액션 (릴리스 전 필수)**: 서명 개인키 `src-tauri/recipe-signing-secret.key`(비커밋, 로컬 단일본)를 안전한 곳에 백업. 분실 시 원격 레시피 갱신 기능 영구 불능
2. M4(플러그인 5종 레시피) 착수. 이월 백로그에 스모크 발견 항목 추가됨 (아래)
